...
// Wir greifen direkt auf die Hardware-Funktionen zu
extern void bm1370_set_nonce_range(uint32_t min, uint32_t max);
extern void SYSTEM_init_system(void *gs);

// Ein simpler Puffer, damit der Linker nicht meckert
uint8_t global_gs_buffer[4096]; 
...
void app_main(void) {
    ...
    // Übergabe an das Restsystem
    SYSTEM_init_system(global_gs_buffer);
}
